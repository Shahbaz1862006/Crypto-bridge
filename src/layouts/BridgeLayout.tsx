...
type BridgeRouteConfig = { step: number; fallbackTo: string; hideStepper?: boolean };

const BRIDGE_ROUTE_CONFIG: Record<string, BridgeRouteConfig> = {
  [ROUTES.BRIDGE.VERIFY_EMAIL]: { step: 0, fallbackTo: ROUTES.MERCHANT.HOME, hideStepper: true },
  [ROUTES.BRIDGE.VERIFY_OTP]: { step: 0, fallbackTo: ROUTES.BRIDGE.VERIFY_EMAIL, hideStepper: true },
...
function getRouteConfig(pathname: string): BridgeRouteConfig {
...
export function BridgeLayout() {
  ...
  const { step, fallbackTo, hideStepper } = getRouteConfig(location.pathname);
